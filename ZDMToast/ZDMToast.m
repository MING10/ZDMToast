...
//  
//
//  Created by zhengdongming on 15/3/25.
...
- (id)initWithText:(NSString *)text_{
    if (self = [super init]) {
        ...
        CGSize size = CGSizeMake(280,MAXFLOAT); //设置一个行高上限
        NSDictionary *attribute = @{NSFontAttributeName: font};
        CGSize textSize = [text boundingRectWithSize:size options: NSStringDrawingTruncatesLastVisibleLine |NSStringDrawingUsesLineFragmentOrigin| NSStringDrawingUsesFontLeading attributes:attribute context:nil].size;
...
- (void)showInView:(UIView*)view{
    ...
    contentView.center = CGPointMake(view.frame.size.width/2, view.frame.size.height/2);
    [self showAnimation];
    [self performSelector:@selector(hideAnimation) withObject:nil afterDelay:duration];
    
}
- (void)showHorizontalInView:(UIView*)view{
    [view  addSubview:contentView];
    contentView.center = CGPointMake(view.center.y, view.center.x);
    [self showAnimation];
    [self performSelector:@selector(hideAnimation) withObject:nil afterDelay:duration];
